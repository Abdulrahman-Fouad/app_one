...
class Property(models.Model):
    ...
    diff = fields.Float(compute='_compute_diff', tracking=True)
    bedrooms = fields.Integer(required=True, tracking=True)
    living_area = fields.Integer(tracking=True)
    facades = fields.Integer(tracking=True)
    # garage = fields.Boolean(tracking=True, groups='app_one.property_manger_group')
    ...
    tag_ids = fields.Many2many('tag', tracking=True)

    description = fields.Text(tracking=True)

    owner_id = fields.Many2one('owner', tracking=True)
    owner_address = fields.Char(related="owner_id.address", tracking=True)
    owner_phone = fields.Char(related="owner_id.phone", tracking=True)

    line_ids = fields.One2many('property.line', 'property_id', tracking=True)
